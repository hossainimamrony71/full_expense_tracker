...
class FinanceReportView(View):
    ...
    def post(self, request):
        ...
        # For the new format, we only support the Transaction model
        ...
        if date_range_option == 'today':
            ...
            computed_end_date = datetime.date(today.year, 12, 31)
            end_date = computed_end_date if computed_end_date <= today else today
        ...
        # Overall header row (Row 1) in Times New Roman; merge A1:E1 for 5 columns
        ...
        # Filter summary (Row 2), merge A2:E2
        ...
        # Define styles for the daily financial summary section
        ...
        for i in range(delta.days + 1):
            ...
            # -- Table Headers for Transactions (Arial) --
            # Merge Category and Subcategory into "Particulars"
            ...
            transactions = Transaction.objects.filter(created_at__date=current_date)
            if user_type:
            ...
            for txn in transactions:
                ...
                # Update overall category totals for potential use (using category as key)
                if txn.category:
                    key = str(txn.category)
                    overall_category_totals[key] = overall_category_totals.get(key, 0) + amount
                current_row += 1

            # -- Subtotal Row for Transactions (Times New Roman) --
            ...
            # -- Money Allocations for the current date --
            money_allocations = MoneyAllocation.objects.filter(created_at__date=current_date)
            if user_type:
                money_allocations = money_allocations.filter(allocated_to__user_type=user_type)
            daily_alloc_total = money_allocations.aggregate(total=Sum('amount'))['total'] or 0

            # -- Compute Daily Totals --
            total_amount = opening_balance + float(daily_alloc_total)
            closing_balance = total_amount - daily_transaction_total

            # -- Aggregations Section Header for Daily Financial Summary (Times New Roman, centered over 2 columns) --
            ...
            # Write summary details (using Times New Roman)
            summary_data = [
            ...
            for label, value in summary_data:
                ...
                ws.cell(row=current_row, column=2, value=value)\
                  .font = Font(name="Times New Roman", bold=True)
                # Apply fill and border to these cells
    ...
    def get_opening_balance(self, date, user_type=None):
        ...
        opening_balance = total_allocations - total_transactions
...
def dashboard(request):
  ...
  # === Bar Chart Data for Money Allocations (Last 6 Months) ===
    ...
    # Convert querysets into dictionaries keyed by month in "YYYY-MM" format
    ted_dict = {item['month'].strftime("%Y-%m"): item['total'] for item in ted_allocations}
    s2l_dict = {item['month'].strftime("%Y-%m"): item['total'] for item in s2l_allocations}

    # Prepare the labels and data lists for the chart
